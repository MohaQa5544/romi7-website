import { and, asc, eq, like, inArray, isNull, notInArray, type SQL } from "drizzle-orm";
...
export default async function FilesSearchPage({ searchParams }: { searchParams: SP }) {
  ...
  const isMockUnit = unit === MOCK_UNIT_KEY;
  ...
  if (isMockUnit) {
    // Comprehensive mock exams: not tied to any unit, but tied to a semester (book).
    conds.push(isNull(schema.files.unitId));
    if (semester) conds.push(eq(schema.files.semesterId, semester));
    if (type === "mock_exam" || type === "mock_exam_solution") {
      conds.push(eq(schema.files.type, type));
    } else {
      conds.push(inArray(schema.files.type, [...MOCK_EXAM_TYPES]));
    }
  } else {
    // Regular unit/semester listing — exclude mock-exam types.
    conds.push(notInArray(schema.files.type, [...MOCK_EXAM_TYPES]));
    if (unit) {
  ...
  return (
    <div className="space-y-8">
      ...
      <FileSearchFilters
        ...
        units={[
          ...unitsForSemester.map((u) => ({
            id: u.id,
            label: `الوحدة ${u.number} — ${u.nameAr}`,
          })),
          { id: MOCK_UNIT_KEY, label: MOCK_UNIT_LABEL },
        ]}
        types={Object.entries(FILE_TYPE_META)
          .filter(([v]) => v !== "mock_exam" && v !== "mock_exam_solution")
          .map(([v, m]) => ({ id: v, label: m.labelAr }))}
        mockTypes={[
          { id: "mock_exam", label: FILE_TYPE_META.mock_exam.labelAr },
          { id: "mock_exam_solution", label: FILE_TYPE_META.mock_exam_solution.labelAr },
        ]}
        mockUnitKey={MOCK_UNIT_KEY}
